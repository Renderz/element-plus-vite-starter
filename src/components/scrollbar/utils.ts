...
>(option: BuildPropOption<T, D, R, V, C>, key?: string): BuildPropReturn<T, D, R, V, C> {
  ...
  if (!isObject(option) || !!option[propKey as keyof BuildPropOption<T, D, R, V, C>]) return option as any;
  ...
  const prop: any = {
    type:
      isObject(type) && Object.getOwnPropertySymbols(type).includes(wrapperKey) ? type[wrapperKey as keyof T] : type,
    required: !!required,
